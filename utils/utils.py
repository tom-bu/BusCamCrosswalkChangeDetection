...
def project_3d_points(points_id, extrinsic, parameters, force_pinhole = False, img_plane = False):
    """
    Parameters
    ----------
    ...
    img_plane: bool
    return all points in the image plane regardless of its in the image or not
    ...
    if img_plane:
        inds = np.where((points_in_camera_frame[:, 2] >= 0))[0]
        pixel = pixel[inds, :]         
        return pixel
